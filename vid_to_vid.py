...
bounds = d.multiline_textbbox((0, 0), tempOutput, font=fnt) #[2] and [3] will be width and height of the video
if bounds[3] % 2 == 1:
...
for img_path in img_paths:
    ...
    outImg = Image.new("RGB", (bounds[2], bounds[3]), (0, 0, 0))
    d = ImageDraw.Draw(outImg)
    d.multiline_text((0, 0), output, font=fnt, fill=(255, 255, 255))
    
    #save the text image
    outImg.save("output_frames2\\" + imgName + imgExt)

#--------------------convert text images to video--------------------
print("converting text images to video...")
input_frames = "output_frames2\\frame_%04d.jpg" #take in the text images as the input frames
cmd = [
    "ffmpeg",
    "-framerate", str(framerate), #framerate of input frames
    "-i", input_frames,
    "-i", video_path,
    "-c:v", "h264",  #video codec
    "-pix_fmt", "yuv420p",  # Set the pixel format
    "-c:a", "aac", #audio codec
    "-map", "0:v", #use video stream from 1st input
    "-map", "1:a", #use audio stream from 2nd input
    "output_videos\\" + videoName + "_output.mp4"
]
subprocess.run(cmd)
